...
float UltimaX=0.0f,UltimaY=0.0f;
...
void Animacion(glm::mat4 &model, int tipoAnimacion){
	...
	switch(tipoAnimacion){
		...
		case 2:
			{ float g= 9.8f;
			  float angulito,tiempo,theta,U,V,Vx,Vy,X,Y;
			  //float AnguloBala,TiempoVuelo,VelocidadVuelo;
			  angulito= AnguloBala*toRadians;
			  X = VelocidadVuelo * cos(angulito) * TiempoVuelo;
			  Y = VelocidadVuelo * sin(angulito) * TiempoVuelo -g*TiempoVuelo*TiempoVuelo/2.0f;
			  Vx = VelocidadVuelo * cos(angulito);
			  Vy = VelocidadVuelo * sin(angulito) - g* TiempoVuelo;
			  V = sqrt(Vx*Vx+Vy*Vy);
			  ...
			  if(Activador==false){
				  ...
				  printf("POSICION X %f :",(X)-UltimaX);
				  printf("POSICION Y %f :",(Y/1000.0f)-UltimaY);
				 //printf("POSICION X %f :",X-UltimaX);
				 //printf("POSICION Y %f :",Y-UltimaY);
				trasl1 = glm::translate(identidad, glm::vec3((X)-UltimaX,(Y/1000.0f)-UltimaY, 0.0f));
				TiempoVuelo=TiempoVuelo+1.0f;
				UltimaX=(X);
				UltimaY=(Y/1000.0f);
...
void DibujoICuadrante(){
	...
	//ImGui::SliderFloat("Angulo THETA  XZ" ,&cameraX, -360, 360);
	//ImGui::SliderFloat("Angulo THETA  YZ" ,&cameraX, -360, 360);
	ImGui::Text(" ");
	ImGui::SliderFloat("Velocidad disparo", &VelocidadVueloLOCAL, 0, 100);
	ImGui::SliderFloat("Velocidad Animacion", &VelocidadAnimacionLOCAL, 1, 100);
	//ImGui::SliderFloat("M. Camara Z", &cameraZ, -360, 360);
	ImGui::PushStyleColor(ImGuiCol_Button, {209.0f/255.0f, 116.0f/255.0f, 11.0f/255.0f, 1} );
